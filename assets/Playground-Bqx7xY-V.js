import{r as t,j as e,b as F}from"./index-D_ft3fJ1.js";import{S as M}from"./SEOHead-CCQGSMdV.js";let w=null;function O(){return w||(w=new Worker(new URL("/assets/pyodide.worker-D_OLqx67.js",import.meta.url),{type:"classic"})),w}function L(){const[a,l]=t.useState("idle"),[n,o]=t.useState(""),[r,p]=t.useState(""),[u,m]=t.useState([]),f=t.useRef(null);t.useEffect(()=>(f.current=O(),()=>void 0),[]);const j=t.useCallback((y,v=[])=>{const c=f.current;if(!c)return;l("loading"),o(""),p(""),m([]);const x=[],g=C=>{const{type:S,text:N,message:E,status:_}=C.data;switch(S){case"status":l(_);break;case"stdout":{const k=/__SVG_START__([\s\S]*?)__SVG_END__/g;let h=N,d;for(;(d=k.exec(N))!==null;)x.push(d[1]),h=h.replace(d[0],"");x.length>0&&m([...x]),h.trim()&&o(s=>s+h);break}case"stderr":o(k=>k+N);break;case"error":l("error"),p(E),c.removeEventListener("message",g);break;case"done":l("done"),c.removeEventListener("message",g);break}};c.addEventListener("message",g),c.postMessage({type:"run",code:y,inputs:v})},[]),i=t.useCallback(()=>{l("idle"),o(""),p(""),m([])},[]);return{status:a,output:n,errorMsg:r,svgOutputs:u,runCode:j,resetOutput:i}}const R={idle:{icon:"fa-circle-play",label:"실행 대기",color:"#6B7280"},loading:{icon:"fa-spinner fa-spin",label:"Pyodide 로딩...",color:"#F59E0B"},running:{icon:"fa-gear fa-spin",label:"실행 중...",color:"#3B82F6"},done:{icon:"fa-circle-check",label:"완료",color:"#10B981"},error:{icon:"fa-circle-xmark",label:"오류",color:"#EF4444"}};function I({initialCode:a="",title:l=""}){const[n,o]=t.useState(a),[r,p]=t.useState(!1),[u,m]=t.useState(""),f=t.useRef(null),j=t.useRef(null),{status:i,output:y,errorMsg:v,svgOutputs:c,runCode:x,resetOutput:g}=L(),C=t.useCallback(()=>{j.current&&f.current&&(j.current.scrollTop=f.current.scrollTop)},[]),S=n.split(`
