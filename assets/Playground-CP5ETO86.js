import{r as t,j as e,b as D}from"./index-xm7pZ-z8.js";import{S as M}from"./SEOHead-Ch0maEOk.js";let E=null;function z(){return E||(E=new Worker(new URL("/assets/pyodide.worker-DhMfE5WB.js",import.meta.url),{type:"classic"})),E}function O(){const[a,l]=t.useState("idle"),[n,o]=t.useState(""),[r,d]=t.useState(""),[p,u]=t.useState([]),m=t.useRef(null);t.useEffect(()=>(m.current=z(),()=>void 0),[]);const h=t.useCallback((b,g=[])=>{const c=m.current;if(!c)return;l("loading"),o(""),d(""),u([]);const y=[],f=N=>{const{type:_,text:k,message:S,status:j}=N.data;switch(_){case"status":l(j);break;case"stdout":o(v=>v+k);break;case"svg":y.push(N.data.svg),u([...y]);break;case"stderr":o(v=>v+k);break;case"error":l("error"),d(S),c.removeEventListener("message",f);break;case"done":l("done"),c.removeEventListener("message",f);break}};c.addEventListener("message",f),c.postMessage({type:"run",code:b,inputs:g})},[]),i=t.useCallback(()=>{l("idle"),o(""),d(""),u([])},[]);return{status:a,output:n,errorMsg:r,svgOutputs:p,runCode:h,resetOutput:i}}const P={idle:{icon:"fa-circle-play",label:"실행 대기",color:"#6B7280"},loading:{icon:"fa-spinner fa-spin",label:"Pyodide 로딩...",color:"#F59E0B"},running:{icon:"fa-gear fa-spin",label:"실행 중...",color:"#3B82F6"},done:{icon:"fa-circle-check",label:"완료",color:"#10B981"},error:{icon:"fa-circle-xmark",label:"오류",color:"#EF4444"}};function I({initialCode:a="",title:l=""}){const[n,o]=t.useState(a),[r,d]=t.useState(!1),[p,u]=t.useState(""),m=t.useRef(null),h=t.useRef(null),{status:i,output:b,errorMsg:g,svgOutputs:c,runCode:y,resetOutput:f}=O(),N=t.useCallback(()=>{h.current&&m.current&&(h.current.scrollTop=m.current.scrollTop)},[]),_=n.split(`
