(function(){"use strict";const l="https://cdn.jsdelivr.net/pyodide/v0.27.0/full/",f=["numpy","pandas","scipy","scikit-learn","statsmodels","matplotlib","seaborn","openpyxl","xlrd","regex","pyyaml","jsonschema","beautifulsoup4"];let a=null;async function c(){return a||(importScripts(l+"pyodide.js"),a=await loadPyodide({indexURL:l,stdout:o=>self.postMessage({type:"stdout",text:o}),stderr:o=>self.postMessage({type:"stderr",text:o})}),await a.runPythonAsync(`
import sys, io
class _MockShow:
    @staticmethod
    def show(*a, **kw):
        ...
        plt.close('all')
`),a)}function m(o){const e=new Set,n=[/^\s*import\s+([\w]+)/gm,/^\s*from\s+([\w]+)/gm];for(const i of n){let t;for(;(t=i.exec(o))!==null;){const s=t[1],r={sklearn:"scikit-learn",bs4:"beautifulsoup4",yaml:"pyyaml",cv2:"opencv-python",PIL:"Pillow",np:"numpy",pd:"pandas",sns:"seaborn",plt:"matplotlib"}[s]||s;f.includes(r)&&e.add(r)}}return e.has("seaborn")&&e.add("matplotlib"),[...e]}self.onmessage=async function(o){const{type:e,code:n,inputs:i}=o.data;if(e==="run")try{self.postMessage({type:"status",status:"loading"});const t=await c(),s=m(n);if(s.length>0&&(self.postMessage({type:"stdout",text:`📦 패키지 로딩: ${s.join(", ")}...
