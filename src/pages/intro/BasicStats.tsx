...
# 5-number summary
print(f"\\n[5-Number Summary]")
print(f"  최소: {scores.min()}, Q1: {q1:.0f}, 중앙값: {q2:.0f}, Q3: {q3:.0f}, 최대: {scores.max()}")`

const code4 = `import numpy as np
import matplotlib.pyplot as plt
...
print("- 통계 검정, 머신러닝 알고리즘의 기본 가정")`

const code5 = `import numpy as np
import matplotlib.pyplot as plt
...
print("  원인은 '기온 상승'이라는 제3의 변수!")`

const code6 = `import pandas as pd
import numpy as np
...
if len(outliers) > 0:
    print(outliers[['이름', '부서', '연봉']].to_string(index=False))`

export default function BasicStats() {
  return (
    <>
      <SEOHead title="통계 기초 이해" description="데이터 분석에 필요한 기본 통계 개념을 직관적으로 학습합니다." />
      <section className="page-header">
        <div className="container">
          <h1>통계 기초 이해</h1>
          <p>데이터 분석에 필요한 기본 통계 개념을 직관적으로 학습합니다</p>
        </div>
      </section>
      <section className="section lesson-content">
        <div className="container">
          <div className="playground-body">
            ...
            <h2>통계, 왜 알아야 할까?</h2>
            <p>통계는 데이터를 요약하고 해석하는 도구입니다. 복잡한 수학 공식보다 "데이터가 무엇을 말하는지" 이해하는 것이 중요합니다. 평균, 분산, 상관계수 같은 기초 통계 개념만 알아도 데이터에서 의미 있는 인사이트를 뽑아낼 수 있습니다.</p>

            <h2>평균, 중앙값, 최빈값 — 대표값</h2>
            <p>대표값은 데이터 전체를 하나의 숫자로 요약합니다. 평균은 이상치에 민감하고, 중앙값은 안정적입니다. 상황에 따라 적절한 대표값을 선택해야 합니다.</p>
            <CodeEditor title="대표값: 평균, 중앙값, 최빈값" initialCode={code1} />

            <h2>분산, 표준편차 — 데이터의 흩어짐</h2>
            <p>평균이 같아도 데이터의 흩어진 정도가 다를 수 있습니다. 분산과 표준편차는 데이터가 평균으로부터 얼마나 떨어져 있는지를 나타냅니다.</p>
            <CodeEditor title="분산과 표준편차" initialCode={code2} />

            <h2>백분위수, 사분위수 — 위치 파악</h2>
            <p>백분위수는 "전체에서 내가 어디쯤 위치하는지" 알려줍니다. 사분위수(Q1, Q2, Q3)는 데이터를 4등분하여 분포를 요약합니다.</p>
            <CodeEditor title="백분위수와 사분위수" initialCode={code3} />

            <h2>정규분포 — 종 모양 곡선</h2>
            <p>자연 현상의 많은 데이터는 평균을 중심으로 좌우 대칭인 정규분포를 따릅니다. 68-95-99.7 법칙을 알면 데이터의 분포를 직관적으로 이해할 수 있습니다.</p>
            <CodeEditor title="정규분포와 68-95-99.7 법칙" initialCode={code4} />

            <h2>상관계수 — 변수 간 관계</h2>
            <p>상관계수(r)는 두 변수 간의 선형 관계의 강도와 방향을 -1~+1 사이의 숫자로 나타냅니다. 단, 상관관계가 있다고 인과관계가 있는 것은 아닙니다.</p>
            <CodeEditor title="상관계수와 상관관계" initialCode={code5} />

            <h2>종합 실습 — 실제 데이터로 통계 요약</h2>
            <p>지금까지 배운 통계 개념을 종합하여 실제 데이터를 분석해봅니다. 기술통계, 그룹별 비교, 상관분석, 이상치 탐지를 한 번에 수행합니다.</p>
            <CodeEditor title="종합 실습: 직원 데이터 통계 분석" initialCode={code6} />

            <div className="callout-box">
              <h3>통계 기초 정리</h3>
              <p>대표값(평균/중앙값)으로 데이터의 중심을 파악하고, 분산/표준편차로 흩어짐을 확인합니다. 백분위수로 위치를 파악하고, 상관계수로 변수 간 관계를 분석합니다. 이 네 가지 개념만 확실히 이해하면 데이터 분석의 기초는 탄탄합니다.</p>
            </div>

            <div className="lesson-nav">
              <Link to="/intro/chart-types" className="lesson-nav-btn prev">&larr; 이전: 그래프의 종류</Link>
