...
print("\\n[파레토 분석 - 누적 비율]")
print(df_sorted[['이름', '매출', '누적매출', '누적비율']])`
...
export default function PandasIntro() {
  return (
    <>
      <SEOHead title="Pandas 기초" description="데이터 처리의 핵심 라이브러리 Pandas 사용법을 배웁니다." />
      <section className="page-header">
        <div className="container">
          <h1>Pandas 기초</h1>
          <p>데이터 처리의 핵심 라이브러리 Pandas 사용법을 배웁니다</p>
        </div>
      </section>
      <section className="section lesson-content">
        <div className="container">
          <div className="playground-body">
            ...
            <h2>DataFrame 생성과 탐색</h2>
            <p>DataFrame은 행과 열로 구성된 2차원 테이블입니다. 딕셔너리, CSV 파일, 데이터베이스 등에서 생성할 수 있습니다.</p>
            <CodeEditor title="DataFrame 생성과 탐색" initialCode={code1} />

            <h2>인덱싱: loc, iloc, 불리언</h2>
            <p>loc는 라벨(이름) 기반, iloc는 위치(번호) 기반 인덱싱입니다. 불리언 인덱싱으로 조건에 맞는 데이터를 필터링합니다.</p>
            <CodeEditor title="인덱싱: loc / iloc / 불리언" initialCode={code2} />

            <h2>데이터 수정과 추가: apply, map</h2>
            <p>apply()는 함수를 행/열에 적용하고, map()은 값을 매핑합니다. 새로운 파생 변수를 만들 때 핵심적으로 사용됩니다.</p>
            <CodeEditor title="데이터 수정과 추가: apply, map" initialCode={code3} />

            <h2>groupby 집계</h2>
            <p>groupby()는 SQL의 GROUP BY와 동일한 기능으로, 그룹별 통계를 계산합니다. 데이터 분석에서 가장 많이 사용되는 기능 중 하나입니다.</p>
            <CodeEditor title="groupby 집계" initialCode={code4} />

            <h2>정렬과 랭킹</h2>
            <p>sort_values()로 정렬하고, rank()로 순위를 매깁니다. 파레토 분석(누적 비율)도 쉽게 구현할 수 있습니다.</p>
            <CodeEditor title="정렬과 랭킹" initialCode={code5} />

            <h2>데이터 결합: merge와 concat</h2>
            <p>merge()는 공통 키를 기준으로 두 테이블을 결합(JOIN)하고, concat()은 단순히 행 또는 열 방향으로 합칩니다.</p>
            <CodeEditor title="데이터 결합: merge / concat" initialCode={code6} />

            <div className="callout-box">
              <h3>Pandas 핵심 요약</h3>
              <p>DataFrame은 엑셀의 시트와 유사한 2차원 테이블입니다. read_csv()로 데이터를 읽고, groupby()로 집계하며, loc/iloc로 원하는 데이터를 선택합니다. merge()로 테이블을 결합하면 대부분의 데이터 처리가 가능합니다.</p>
            </div>
