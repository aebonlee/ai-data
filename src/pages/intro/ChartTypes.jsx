...
const code1 = `import matplotlib.pyplot as plt
import numpy as np
...
const code2 = `import matplotlib.pyplot as plt
import numpy as np
...
print("- 영역 그래프: 크기감을 강조할 때 활용")`

const code3 = `import matplotlib.pyplot as plt
...
print("- 주의: 비율 차이가 작으면 비교가 어려움 → 막대 그래프 권장")`

const code4 = `import matplotlib.pyplot as plt
import numpy as np
...
print("- bins 수에 따라 해상도가 달라짐")`

const code5 = `import matplotlib.pyplot as plt
import numpy as np
...
print("- 버블 차트: 3번째 변수를 크기/색상으로 표현")`

const code6 = `import matplotlib.pyplot as plt
import numpy as np
...
for dept, data in dept_sales.items():
    ...
    print(f"{dept} — 중앙값: {med:.0f}, Q1: {q1:.0f}, Q3: {q3:.0f}")`

const code7 = `import pandas as pd
import numpy as np
...
print("  ✓ 색상은 3~5가지 이내로 제한")`

export default function ChartTypes() {
  return (
    <>
      <SEOHead title="그래프의 종류" description="데이터 분석에서 사용하는 주요 그래프 유형을 기초 수준으로 학습합니다." />
      <section className="page-header">
        <div className="container">
          <h1>그래프의 종류</h1>
          <p>데이터 분석에서 사용하는 주요 그래프 유형과 활용법을 학습합니다</p>
        </div>
      </section>
      <section className="section lesson-content">
        <div className="container">
          <div className="playground-body">
            ...
            <h2>왜 그래프가 중요한가?</h2>
            <p>숫자만으로는 데이터의 패턴을 파악하기 어렵습니다. 그래프는 데이터를 시각적으로 표현하여 트렌드, 비교, 분포, 관계를 직관적으로 이해할 수 있게 해줍니다. 목적에 맞는 그래프를 선택하는 것이 효과적인 데이터 분석의 첫걸음입니다.</p>

            <h2>막대 그래프 (Bar Chart)</h2>
            <p>범주(카테고리)별 값을 비교할 때 가장 많이 사용하는 그래프입니다. 세로 막대는 기본 비교에, 가로 막대는 항목명이 길 때 적합합니다.</p>
            <CodeEditor title="막대 그래프 — 범주 비교" initialCode={code1} />

            <h2>선 그래프 (Line Chart)</h2>
            <p>시간에 따른 데이터의 변화(트렌드)를 보여줄 때 사용합니다. 여러 시리즈를 비교하거나, 영역 그래프로 크기감을 강조할 수 있습니다.</p>
            <CodeEditor title="선 그래프 — 시간에 따른 변화" initialCode={code2} />

            <h2>파이 차트 (Pie Chart)</h2>
            <p>전체에서 각 항목이 차지하는 비율을 보여줍니다. 항목이 5~6개 이하일 때 효과적이며, 도넛 차트로 변형할 수 있습니다.</p>
            <CodeEditor title="파이 차트 — 비율/구성" initialCode={code3} />

            <h2>히스토그램 (Histogram)</h2>
            <p>연속형 데이터의 분포(빈도)를 확인할 때 사용합니다. 막대 그래프와 비슷해 보이지만, X축이 연속적인 구간이라는 차이가 있습니다.</p>
            <CodeEditor title="히스토그램 — 분포 확인" initialCode={code4} />

            <h2>산점도 (Scatter Plot)</h2>
            <p>두 변수 간의 관계(상관관계)를 파악할 때 사용합니다. 추세선으로 방향과 강도를 확인하고, 버블 차트로 3번째 변수를 표현할 수 있습니다.</p>
            <CodeEditor title="산점도 — 상관관계" initialCode={code5} />

            <h2>박스플롯 (Box Plot)</h2>
            <p>데이터의 분포를 요약하고 이상치를 한눈에 파악할 때 사용합니다. 중앙값, 사분위수, 이상치를 모두 보여주는 강력한 그래프입니다.</p>
            <CodeEditor title="박스플롯 — 분포 요약/이상치" initialCode={code6} />

            <h2>어떤 그래프를 언제 써야 할까?</h2>
            <p>데이터의 유형과 분석 목적에 따라 적절한 그래프를 선택해야 합니다. 아래 가이드를 참고하세요.</p>
            <CodeEditor title="그래프 선택 가이드 — 종합 비교" initialCode={code7} />

            <div className="callout-box">
              <h3>그래프 선택 팁</h3>
              <p>그래프는 "데이터를 예쁘게 꾸미는 것"이 아니라 "메시지를 명확하게 전달하는 것"이 목적입니다. 하나의 그래프에 하나의 메시지를 담고, 불필요한 장식은 최소화하세요. 막대 그래프와 선 그래프만 잘 활용해도 대부분의 분석을 효과적으로 전달할 수 있습니다.</p>
            </div>

            <div className="lesson-nav">
              <Link to="/intro/data-types" className="lesson-nav-btn prev">&larr; 이전: 데이터 유형 이해</Link>
              <Link to="/intro/basic-stats" className="lesson-nav-btn next">다음: 통계 기초 이해 &rarr;</Link>
            </div>
          </div>
        </div>
      </section>
