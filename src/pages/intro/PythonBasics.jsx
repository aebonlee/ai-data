...
print(f"\\n단가: {price:,}원 x {quantity}개 = {total:,}원")`

const code2 = `# 리스트 (List) - 순서가 있는 변경 가능한 컬렉션
...
for emp in employees:
    print(f"  {emp['이름']} ({emp['부서']}): {emp['연봉']:,}만원")`
...
print("\\n[가변 인수]")
summary(1, 2, 3, name="분석", level="초급")`

const code5 = `# 모듈과 NumPy 기초
import numpy as np
    ...
    <>
      ...
      <section className="page-header">
        <div className="container">
          <h1>Python 기초</h1>
          <p>데이터 분석을 위한 Python 프로그래밍 기초를 학습합니다</p>
        </div>
      </section>
      <section className="section lesson-content">
        <div className="container">
          <div className="playground-body">
            ...
            <h2>Python이란?</h2>
            <p>Python은 간결하고 읽기 쉬운 문법으로 데이터 분석, 인공지능, 웹 개발 등 다양한 분야에서 사용되는 프로그래밍 언어입니다. 특히 풍부한 데이터 관련 라이브러리 생태계(Pandas, NumPy, Matplotlib, Scikit-learn) 덕분에 데이터 분석의 표준 언어로 자리잡았습니다.</p>

            <h2>변수와 자료형</h2>
            <p>Python의 기본 자료형은 정수(int), 실수(float), 문자열(str), 불리언(bool)입니다. 변수에 값을 대입하면 자동으로 타입이 결정됩니다.</p>
            <CodeEditor title="변수와 자료형" initialCode={code1} />

            <h2>리스트와 딕셔너리</h2>
            <p>리스트는 순서가 있는 데이터 모음이고, 딕셔너리는 키-값 쌍으로 구성됩니다. 데이터 분석에서 가장 많이 사용되는 자료구조입니다.</p>
            <CodeEditor title="리스트와 딕셔너리" initialCode={code2} />

            <h2>조건문과 반복문</h2>
            <p>조건문(if/elif/else)으로 분기를 처리하고, 반복문(for/while)으로 데이터를 순회합니다. 리스트 컴프리헨션은 Python만의 강력한 문법입니다.</p>
            <CodeEditor title="조건문과 반복문" initialCode={code3} />

            <h2>함수와 람다</h2>
            <p>함수를 정의하여 코드를 재사용하고, 람다 함수로 간단한 연산을 한 줄로 표현합니다. 데이터 분석에서 apply(), sorted() 등과 함께 람다를 자주 사용합니다.</p>
            <CodeEditor title="함수와 람다" initialCode={code4} />

            <h2>모듈과 NumPy 기초</h2>
            <p>NumPy는 Python 과학 계산의 핵심 라이브러리입니다. 대규모 배열 연산을 빠르게 처리하며, Pandas의 기반이 됩니다.</p>
            <CodeEditor title="모듈과 NumPy 기초" initialCode={code5} />
            ...
            <div className="lesson-nav">
              ...
              <Link to="/intro/pandas-intro" className="lesson-nav-btn next">다음: Pandas 기초 &rarr;</Link>
            </div>
          </div>
        </div>
      </section>
