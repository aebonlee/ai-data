...
print(f"\\n[지역별 평균 가격]")
print(df.groupby('지역')['가격'].mean().sort_values(ascending=False).round(0))`
...
print("\\n해석: 면적 1단위 증가 시 가격이 약 {:.0f} 증가".format(coef_df[coef_df['피처']=='면적']['계수'].values[0]))`
...
for feat, imp in top.items():
    print(f"  {feat}: {imp:.3f}")
print(f"\\n→ '{top.index[0]}'이(가) 가격 예측에 가장 큰 영향을 미칩니다.")`

export default function Regression() {
  return (
    <>
      <SEOHead title="회귀 분석" description="회귀 알고리즘으로 연속값을 예측하는 머신러닝을 학습합니다." />
      <section className="page-header">
        <div className="container">
          <h1>회귀 분석</h1>
          <p>회귀 알고리즘으로 연속값을 예측하는 머신러닝을 학습합니다</p>
        </div>
      </section>
      <section className="section lesson-content">
        <div className="container">
          <div className="playground-body">
            ...
            <h2>회귀란?</h2>
            <p>회귀(Regression)는 연속적인 숫자 값을 예측하는 지도학습 기법입니다. 분류가 "어떤 범주?"를 예측한다면, 회귀는 "얼마?"를 예측합니다.</p>
            <ul>
              <li><strong>주택 가격 예측</strong> — 면적, 위치, 층수 등으로 가격 예측</li>
              <li><strong>매출 예측</strong> — 광고비, 시즌, 프로모션으로 매출 예측</li>
              <li><strong>수요 예측</strong> — 과거 데이터로 미래 수요 예측</li>
              <li><strong>연봉 예측</strong> — 경력, 학력, 직무로 연봉 예측</li>
            </ul>

            <h2>데이터 생성: 주택 가격 예측</h2>
            <p>300건의 주택 데이터를 생성합니다. 면적, 방수, 층수, 역거리, 건축년도, 지역 정보로 가격을 예측합니다.</p>
            <CodeEditor title="1단계: 데이터 생성 및 탐색" initialCode={code1} />

            <h2>전처리: 인코딩, 상관분석</h2>
            <p>범주형 변수(지역)를 인코딩하고, 타겟(가격)과의 상관관계를 분석하여 중요한 피처를 파악합니다.</p>
            <CodeEditor title="2단계: 전처리 및 상관분석" initialCode={code2} />

            <h2>선형 회귀: 계수 해석</h2>
            <p>선형 회귀는 가장 기본적인 회귀 모델입니다. 각 피처의 계수를 통해 가격에 미치는 영향을 정량적으로 해석할 수 있습니다.</p>
            <CodeEditor title="3단계: 선형 회귀" initialCode={code3} />

            <h2>의사결정나무 회귀</h2>
            <p>의사결정나무는 비선형 관계도 잡아낼 수 있습니다. 피처 중요도를 통해 어떤 변수가 예측에 중요한지 파악합니다.</p>
            <CodeEditor title="4단계: 의사결정나무 회귀" initialCode={code4} />

            <h2>랜덤포레스트 회귀 + 모델 비교</h2>
            <p>3가지 회귀 모델의 RMSE와 R2를 비교하고, 최적 모델의 예측 결과를 시각화합니다.</p>
            <CodeEditor title="5단계: 랜덤포레스트 + 모델 비교" initialCode={code5} />

            <h2>평가: RMSE/R2, 잔차 분석, Feature Importance</h2>
            <p>RMSE(오차 크기), R2(설명력), 잔차 분석으로 모델의 품질을 종합적으로 평가합니다.</p>
            <CodeEditor title="6단계: 종합 평가" initialCode={code6} />

            <div className="callout-box">
              <h3>회귀 평가 지표</h3>
              <p>RMSE: 낮을수록 좋음 (예측 오차의 크기) / MAE: 낮을수록 좋음 (평균 절대 오차) / R2: 1에 가까울수록 좋음 (모델의 설명력). 잔차가 정규분포를 따르고 예측값과 무관하면 좋은 모델입니다.</p>
            </div>

            <div className="lesson-nav">
              <Link to="/learn/classification" className="lesson-nav-btn prev">&larr; 이전: 분류 분석</Link>
