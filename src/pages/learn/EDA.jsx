...
const code7 = `import matplotlib.pyplot as plt
import numpy as np
import pandas as pd
...
print(f"평균 만족도: {df['만족도'].mean():.1f}/5.0")`
    ...
    <>
      ...
      <section className="page-header">
        <div className="container">
          ...
          <p>데이터의 패턴과 인사이트를 탐색하고 발견하는 방법을 학습합니다</p>
        </div>
      </section>
      <section className="section lesson-content">
        <div className="container">
          <div className="playground-body">
            ...
            <h2>데이터 개요 파악</h2>
            <p>데이터의 크기, 타입, 기술통계, 결측치, 고유값을 먼저 확인합니다.</p>
            <CodeEditor title="데이터 개요 파악" initialCode={code1} />

            <h2>단변량 분석: 히스토그램, KDE, 박스플롯</h2>
            <p>각 변수의 분포를 개별적으로 파악합니다. 수치형은 히스토그램과 박스플롯으로 분포와 이상치를 확인합니다.</p>
            <CodeEditor title="단변량 분석: 수치형 변수" initialCode={code2} />

            <h2>범주형 분석: value_counts, 파이차트</h2>
            <p>범주형 변수의 빈도와 비율을 파악합니다. 불균형한 범주가 있는지 확인합니다.</p>
            <CodeEditor title="단변량 분석: 범주형 변수" initialCode={code3} />

            <h2>이변량 분석: scatter, boxplot, crosstab</h2>
            <p>두 변수 간의 관계를 분석합니다. 산점도, 그룹별 박스플롯, 교차표로 패턴을 발견합니다.</p>
            <CodeEditor title="이변량 분석" initialCode={code4} />

            <h2>상관분석 히트맵</h2>
            <p>모든 수치형 변수 간의 상관관계를 한눈에 파악합니다. |r| &gt; 0.5이면 강한 상관, 0.3~0.5이면 중간 상관입니다.</p>
            <CodeEditor title="상관분석 히트맵" initialCode={code5} />

            <h2>피벗테이블과 그룹별 비교</h2>
            <p>그룹별 통계를 피벗테이블과 교차표로 요약합니다.</p>
            <CodeEditor title="피벗테이블과 그룹별 비교" initialCode={code6} />

            <h2>종합 EDA 대시보드</h2>
            <p>여러 분석 결과를 하나의 대시보드로 구성하여 전체적인 인사이트를 정리합니다.</p>
            <CodeEditor title="종합 EDA 대시보드" initialCode={code7} />

            <div className="callout-box">
              <h3>EDA 핵심 질문</h3>
              <p>1) 데이터의 크기와 구조는? 2) 결측치와 이상치가 있는가? 3) 각 변수의 분포는 어떤 형태인가? 4) 변수 간 어떤 상관관계가 있는가? 5) 그룹별로 차이가 있는가? 이 질문들에 답하면 EDA가 완성됩니다.</p>
            </div>

            <div className="lesson-nav">
              <Link to="/learn/preprocessing" className="lesson-nav-btn prev">&larr; 이전: 데이터 전처리</Link>
              <Link to="/learn/statistics" className="lesson-nav-btn next">다음: 통계 분석 &rarr;</Link>
            </div>
          </div>
        </div>
      </section>
