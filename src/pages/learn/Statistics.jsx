...
print(f"  → {'뾰족' if kurtosis(scores) > 0 else '평평' if kurtosis(scores) < 0 else '정규분포'}")`
...
print("포아송분포: 이산형, 단위시간당 사건 발생 횟수, 고객방문/콜수")`
...
print(f"\\n해석: 모평균이 ({ci_95[0]:.1f}, {ci_95[1]:.1f})에 있을 확률이 95%")`
...
print(f"결론: {'유의한 향상' if p_value2 < 0.05 else '유의한 변화 없음'} (α=0.05)")`
...
print("어떤 그룹이 다른지는 사후검정(Tukey HSD)으로 확인합니다.")`

const code6 = `import numpy as np
import pandas as pd
from scipy import stats
...
print(f"\\n[기대빈도]")
print(pd.DataFrame(expected, index=cross.index, columns=cross.columns).round(1))`

const code7 = `import numpy as np
import pandas as pd
from scipy import stats
import matplotlib.pyplot as plt
...
export default function Statistics() {
  return (
    <>
      <SEOHead title="통계 분석" description="데이터 분석에 필요한 통계 개념과 검정을 학습합니다." />
      <section className="page-header">
        <div className="container">
          <h1>통계 분석</h1>
          <p>데이터 분석에 필요한 통계 개념과 검정을 학습합니다</p>
        </div>
      </section>
      <section className="section lesson-content">
        <div className="container">
          <div className="playground-body">
            ...
            <h2>기술통계: 중심, 산포, 형태</h2>
            <p>데이터의 특성을 숫자로 요약합니다. 중심경향(평균, 중앙값), 산포도(분산, 표준편차), 형태(왜도, 첨도)를 파악합니다.</p>
            <CodeEditor title="기술통계: 중심경향, 산포도, 형태" initialCode={code1} />

            <h2>확률분포: 정규, 이항, 포아송</h2>
            <p>확률분포는 데이터의 패턴을 수학적으로 모델링합니다. 정규분포는 가장 기본이 되는 연속형 분포입니다.</p>
            <CodeEditor title="주요 확률분포" initialCode={code2} />

            <h2>신뢰구간</h2>
            <p>표본에서 모집단의 모수를 추정할 때, 추정값의 불확실성을 구간으로 표현합니다.</p>
            <CodeEditor title="신뢰구간 추정" initialCode={code3} />

            <h2>t-검정</h2>
            <p>두 그룹의 평균이 통계적으로 유의하게 다른지 검정합니다. 독립표본(서로 다른 그룹)과 대응표본(동일 그룹의 전후)이 있습니다.</p>
            <CodeEditor title="t-검정: 두 그룹 평균 비교" initialCode={code4} />

            <h2>ANOVA (분산분석)</h2>
            <p>3개 이상 그룹의 평균 차이를 검정합니다. t-검정의 확장 버전으로, 여러 그룹을 한 번에 비교합니다.</p>
            <CodeEditor title="ANOVA: 3개 이상 그룹 비교" initialCode={code5} />

            <h2>카이제곱 검정</h2>
            <p>범주형 변수 간의 독립성을 검정합니다. "성별에 따라 선호 채널이 다른가?"와 같은 질문에 답합니다.</p>
            <CodeEditor title="카이제곱 검정: 독립성 검정" initialCode={code6} />

            <h2>상관분석: Pearson vs Spearman</h2>
            <p>두 연속형 변수 간의 관계 강도를 측정합니다. Pearson은 선형 관계, Spearman은 순위(비선형) 관계를 측정합니다.</p>
            <CodeEditor title="상관분석: Pearson vs Spearman" initialCode={code7} />

            <div className="callout-box">
              <h3>통계 검정 선택 가이드</h3>
              <p>수치형 2그룹 비교 → t-검정 / 수치형 3그룹+ 비교 → ANOVA / 범주형 독립성 → 카이제곱 / 수치형 관계 → 상관분석. p-value &lt; 0.05이면 통계적으로 유의합니다.</p>
            </div>

            <div className="lesson-nav">
              <Link to="/learn/eda" className="lesson-nav-btn prev">&larr; 이전: 탐색적 데이터 분석</Link>
              <Link to="/learn/visualization" className="lesson-nav-btn next">다음: 시각화 기법 &rarr;</Link>
            </div>
          </div>
        </div>
      </section>
