...
export default function Classification() {
  return (
    <>
      <SEOHead title="분류 분석" description="분류 알고리즘으로 범주를 예측하는 머신러닝을 학습합니다." />
      <section className="page-header">
        <div className="container">
          <h1>분류 분석</h1>
          <p>분류 알고리즘으로 범주를 예측하는 머신러닝을 학습합니다</p>
        </div>
      </section>
      <section className="section lesson-content">
        <div className="container">
          <div className="playground-body">
            ...
            <h2>분류란?</h2>
            <p>분류(Classification)는 데이터를 미리 정의된 범주(클래스)로 나누는 지도학습 기법입니다. 이진 분류(합격/불합격)와 다중 분류(A/B/C 등급)가 있습니다.</p>
            <ul>
              <li><strong>이진 분류</strong> — 이직 예측(유지/이직), 스팸 탐지(정상/스팸), 질병 진단(양성/음성)</li>
              <li><strong>다중 분류</strong> — 고객 등급(VIP/일반/신규), 제품 카테고리 분류, 감성 분석(긍정/중립/부정)</li>
            </ul>

            <h2>데이터 생성: 직원 이직 예측</h2>
            <p>300명의 직원 데이터를 생성합니다. 나이, 경력, 연봉, 만족도, 야근시간 등의 피처로 이직 여부를 예측합니다.</p>
            <CodeEditor title="1단계: 데이터 생성 및 탐색" initialCode={code1} />

            <h2>전처리: LabelEncoder, train_test_split</h2>
            <p>범주형 변수를 숫자로 인코딩하고, 데이터를 학습용(80%)과 테스트용(20%)으로 분리합니다.</p>
            <CodeEditor title="2단계: 전처리 및 데이터 분리" initialCode={code2} />

            <h2>로지스틱 회귀</h2>
            <p>로지스틱 회귀는 가장 기본적인 분류 알고리즘입니다. 각 피처의 계수를 통해 어떤 요인이 이직에 영향을 미치는지 해석할 수 있습니다.</p>
            <CodeEditor title="3단계: 로지스틱 회귀" initialCode={code3} />

            <h2>의사결정나무</h2>
            <p>의사결정나무는 데이터를 조건에 따라 분기하여 분류합니다. 직관적이고 해석이 쉬운 것이 장점입니다.</p>
            <CodeEditor title="4단계: 의사결정나무" initialCode={code4} />

            <h2>랜덤포레스트 + 모델 비교</h2>
            <p>랜덤포레스트는 여러 의사결정나무를 앙상블하여 성능을 높입니다. 3가지 모델의 성능을 비교합니다.</p>
            <CodeEditor title="5단계: 랜덤포레스트 + 모델 비교" initialCode={code5} />

            <h2>평가: confusion matrix, classification_report</h2>
            <p>혼동 행렬로 예측 결과를 상세히 분석하고, precision, recall, f1-score로 모델 성능을 평가합니다.</p>
            <CodeEditor title="6단계: 모델 평가 + Feature Importance" initialCode={code6} />

            <div className="callout-box">
              <h3>분류 모델 선택 가이드</h3>
              <p>해석력이 중요하면 → 로지스틱 회귀 / 비선형 관계가 있으면 → 의사결정나무 / 최고 성능이 필요하면 → 랜덤포레스트. 실무에서는 여러 모델을 비교한 후 최적 모델을 선택합니다.</p>
            </div>

            <div className="lesson-nav">
              <Link to="/learn/visualization" className="lesson-nav-btn prev">&larr; 이전: 시각화 기법</Link>
              <Link to="/learn/regression" className="lesson-nav-btn next">다음: 회귀 분석 &rarr;</Link>
            </div>
          </div>
        </div>
      </section>
