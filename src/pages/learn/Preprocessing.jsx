...
print(f"완전한 행 수: {df.dropna().shape[0]} / {df.shape[0]}")`
...
print(f"중앙값 대체 - 이상치에 강건, 분포 왜곡 적음")`
...
print(f"{'윈저라이즈':<12} {clean2.mean():>8.1f} {clean2.median():>8.1f} {clean2.std():>8.1f}")`
...
print("get_dummies: 순서가 없는 범주 (부서, 지역, 색상)")`
...
print("4. 텍스트 길이, 단어 수 등 파생 변수")`

export default function Preprocessing() {
  return (
    <>
      <SEOHead title="데이터 전처리" description="결측치 처리, 이상치 탐지, 인코딩, 피처 엔지니어링을 학습합니다." />
      <section className="page-header">
        <div className="container">
          <h1>데이터 전처리</h1>
          <p>결측치 처리, 이상치 탐지, 데이터 변환 기법을 학습합니다</p>
        </div>
      </section>
      <section className="section lesson-content">
        <div className="container">
          <div className="playground-body">
            ...
            <h2>전처리의 중요성</h2>
            <p>실무 데이터는 결측치, 이상치, 불일치 등 다양한 문제를 포함합니다. "Garbage In, Garbage Out" — 데이터 품질이 분석 결과의 품질을 결정합니다. 전처리는 전체 분석 시간의 60~80%를 차지할 만큼 중요한 단계입니다.</p>

            <h2>결측치 탐지와 시각화</h2>
            <p>결측치(Missing Value)는 데이터에 값이 없는 경우입니다. isnull()과 notnull()로 탐지하고, 비율을 확인합니다.</p>
            <CodeEditor title="결측치 탐지와 현황 파악" initialCode={code1} />

            <h2>결측치 처리 전략: 삭제 vs 대체</h2>
            <p>결측치가 적으면 삭제, 많으면 대체가 일반적입니다. 데이터의 특성과 결측 패턴에 따라 최적의 방법을 선택합니다.</p>
            <CodeEditor title="결측치 처리: 삭제 vs 대체 비교" initialCode={code2} />

            <h2>이상치 탐지: IQR, Z-Score</h2>
            <p>이상치(Outlier)는 데이터의 일반적인 패턴에서 크게 벗어난 값입니다. IQR 방식과 Z-Score 방식으로 탐지합니다.</p>
            <CodeEditor title="이상치 탐지: IQR vs Z-Score" initialCode={code3} />

            <h2>이상치 처리: 제거, 윈저라이즈, 로그변환</h2>
            <p>이상치를 발견했다면 제거, 경계값 대체(윈저라이즈), 변환(로그) 중 적절한 방법을 선택합니다.</p>
            <CodeEditor title="이상치 처리 전략 비교" initialCode={code4} />

            <h2>인코딩: LabelEncoder vs get_dummies</h2>
            <p>머신러닝 모델은 숫자만 처리할 수 있으므로, 범주형 데이터를 숫자로 변환해야 합니다.</p>
            <CodeEditor title="범주형 인코딩 방법" initialCode={code5} />

            <h2>피처 엔지니어링 기초</h2>
            <p>기존 변수에서 새로운 의미 있는 변수를 만드는 과정입니다. 좋은 피처가 모델 성능을 결정합니다.</p>
            <CodeEditor title="피처 엔지니어링 기초" initialCode={code6} />

            <div className="callout-box">
              <h3>전처리 체크리스트</h3>
              <p>1) 결측치 비율 확인 → 2) 이상치 탐지 및 처리 → 3) 데이터 타입 변환 → 4) 범주형 인코딩 → 5) 스케일링 → 6) 피처 엔지니어링. 이 순서를 따르면 체계적인 전처리가 가능합니다.</p>
            </div>
