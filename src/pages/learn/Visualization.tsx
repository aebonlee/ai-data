...
export default function Visualization() {
  return (
    <>
      <SEOHead title="시각화 기법" description="Matplotlib, Seaborn을 활용한 데이터 시각화를 학습합니다." />
      <section className="page-header">
        <div className="container">
          <h1>시각화 기법</h1>
          <p>Matplotlib, Seaborn을 활용한 데이터 시각화를 학습합니다</p>
        </div>
      </section>
      <section className="section lesson-content">
        <div className="container">
          <div className="playground-body">
            ...
            <h2>Matplotlib 기초</h2>
            <p>Matplotlib은 Python의 가장 기본적인 시각화 라이브러리입니다. 라인, 바, 파이 차트 등 모든 기본 차트를 지원합니다.</p>
            <CodeEditor title="Matplotlib 기초: 라인, 바, 파이" initialCode={code1} />

            <h2>Matplotlib 고급: subplots, twin axes</h2>
            <p>쌍축 차트로 서로 다른 단위의 데이터를 하나의 차트에 표현하고, 주석(annotation)으로 핵심 포인트를 강조합니다.</p>
            <CodeEditor title="Matplotlib 고급: 쌍축 차트 + 주석" initialCode={code2} />

            <h2>Seaborn 통계 시각화</h2>
            <p>Seaborn은 Matplotlib 기반의 통계 시각화 라이브러리입니다. 더 아름다운 디폴트 스타일과 통계적 기능을 제공합니다.</p>
            <CodeEditor title="Seaborn 통계 시각화" initialCode={code3} />

            <h2>범주형 시각화</h2>
            <p>범주형 변수의 빈도, 비율, 교차 분석을 시각화합니다. 카운트플롯, 스택 바, 히트맵, 포인트플롯 등을 활용합니다.</p>
            <CodeEditor title="범주형 시각화" initialCode={code4} />

            <h2>시계열 시각화</h2>
            <p>시간에 따른 데이터의 변화를 시각화합니다. 이동평균, 월별 집계, 요일별 패턴 등을 표현합니다.</p>
            <CodeEditor title="시계열 시각화" initialCode={code5} />

            <h2>종합 대시보드 구성</h2>
            <p>여러 차트를 하나의 figure에 배치하여 종합적인 분석 대시보드를 구성합니다.</p>
            <CodeEditor title="종합 대시보드 구성" initialCode={code6} />

            <div className="callout-box">
              <h3>시각화 선택 가이드</h3>
              <p>추이 → 라인 차트 / 비교 → 바 차트 / 비중 → 파이 차트 / 분포 → 히스토그램, 박스플롯 / 관계 → 산점도 / 상관관계 → 히트맵. 데이터의 목적에 맞는 차트를 선택하는 것이 핵심입니다.</p>
            </div>

            <div className="lesson-nav">
              <Link to="/learn/statistics" className="lesson-nav-btn prev">&larr; 이전: 통계 분석</Link>
              <Link to="/learn/classification" className="lesson-nav-btn next">다음: 분류 분석 &rarr;</Link>
            </div>
          </div>
        </div>
      </section>
