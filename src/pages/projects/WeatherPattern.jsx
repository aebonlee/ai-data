...
for m, t in monthly_temp.items():
    print(f"  {m:2d}월: {t:5.1f}°C")`

const step3 = `# 이상치 탐지: IQR 방법
...
print(f"연 총 강수량: {df['강수량'].sum():.1f}mm")`

export default function WeatherPattern() {
  return (
    <>
      <SEOHead title="날씨 패턴 분석" description="시계열 기상 데이터에 이동평균을 적용하고 이상치를 탐지합니다." />
      <section className="page-header">
        <div className="container">
          <h1>날씨 패턴 분석</h1>
          <p>시계열, 이동평균, 이상치 탐지로 기상 패턴을 분석합니다</p>
        </div>
      </section>
        ...
        <div className="container">
          <div className="lesson-body">
            ...
            <div className="callout-box">
              <h3>프로젝트 목표</h3>
              <p>1년간의 기상 데이터를 생성하고, 이동평균으로 트렌드를 파악하며 IQR 방법으로 이상치를 탐지합니다.
              <strong> 난이도: 중급</strong> | 핵심 스킬: 시계열, 이동평균, 이상치 탐지</p>
            </div>

            <h2>데이터 설명</h2>
            <p>2024년 1년간의 일별 기상 데이터입니다. 기온은 계절 변동(사인 곡선)을 반영하고, 강수는 확률적으로 발생합니다.</p>
            <ul>
              <li><strong>기온</strong> — 계절별 사인 곡선 + 노이즈 (°C)</li>
              <li><strong>습도</strong> — 계절 변동 반영 (20~100%)</li>
              <li><strong>풍속</strong> — 정규분포 (m/s)</li>
              <li><strong>강수량</strong> — 30% 확률, 지수분포 (mm)</li>
            </ul>

            <h2>1단계: 데이터 생성 및 탐색</h2>
            <p>사인 곡선으로 계절 변동을 반영한 1년 기상 데이터를 생성합니다.</p>
            <CodeEditor title="1단계: 데이터 생성 및 탐색" initialCode={step1} />

            <h2>2단계: 이동평균 분석</h2>
            <p>7일/30일 이동평균으로 기온 트렌드를, 14일 이동평균으로 강수 트렌드를 파악합니다.</p>
            <CodeEditor title="2단계: 이동평균 분석" initialCode={step2} />

            <h2>3단계: 이상치 탐지</h2>
            <p>IQR 방법으로 기온/풍속/강수의 이상치를 탐지하고 시각화합니다.</p>
            <CodeEditor title="3단계: 이상치 탐지 (IQR)" initialCode={step3} />

            <h2>4단계: 종합 분석</h2>
            <p>계절별 기온 분포, 월별 강수, 기온-습도 관계, 계절별 기상 비교를 대시보드로 구성합니다.</p>
            <CodeEditor title="4단계: 종합 대시보드" initialCode={step4} />

            <div className="callout-box">
              <h3>실습 과제</h3>
              <ul>
                <li>Z-score 방법으로 이상치를 탐지하고 IQR 결과와 비교해보세요.</li>
                <li>기온의 주간(7일) 패턴을 분석하고 요일별 평균 기온을 구해보세요.</li>
                <li>풍속과 강수량의 관계를 산점도로 시각화하고 해석해보세요.</li>
              </ul>
            </div>

            <div className="lesson-nav">
              <Link to="/projects/survey-dashboard" className="lesson-nav-btn prev">&larr; 이전: 설문조사 대시보드</Link>
              <Link to="/projects/product-review" className="lesson-nav-btn next">다음: 상품 리뷰 분석 &rarr;</Link>
            </div>
          </div>
        </div>
      </section>
