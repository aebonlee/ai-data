...
print(f"최다 리뷰 상품: {review_counts.idxmax()} ({review_counts.max()}건)")`

export default function ProductReview() {
  return (
    <>
      <SEOHead title="상품 리뷰 분석" description="상품 리뷰 텍스트를 기초 분석하고 다중 시각화합니다." />
      <section className="page-header">
        <div className="container">
          <h1>상품 리뷰 분석</h1>
          <p>텍스트 기초분석과 다중 시각화로 상품 리뷰를 분석합니다</p>
        </div>
      </section>
        ...
        <div className="container">
          <div className="lesson-body">
            ...
            <div className="callout-box">
              <h3>프로젝트 목표</h3>
              <p>5개 상품의 300건 리뷰를 생성하고, 평점 분석, 텍스트 빈도 분석, 감성별 비교를 수행합니다.
              <strong> 난이도: 중급</strong> | 핵심 스킬: 텍스트 기초분석, 다중 시각화</p>
            </div>

            <h2>데이터 설명</h2>
            <p>노트북, 스마트폰, 태블릿, 이어폰, 스마트워치 5개 상품의 리뷰 300건입니다. 평점에 따라 긍정/부정 키워드로 리뷰 텍스트가 생성됩니다.</p>

            <h2>1단계: 데이터 생성 및 탐색</h2>
            <p>평점 기반 리뷰 텍스트를 생성하고 기본 분포를 확인합니다.</p>
            <CodeEditor title="1단계: 데이터 생성 및 탐색" initialCode={step1} />

            <h2>2단계: 평점 분석</h2>
            <p>상품별 평점 비교, 전체 분포, 월별 추이를 시각화합니다.</p>
            <CodeEditor title="2단계: 평점 분석" initialCode={step2} />

            <h2>3단계: 텍스트 기초분석</h2>
            <p>단어 빈도를 집계하고, 평점별 리뷰 길이 차이를 박스플롯으로 비교합니다.</p>
            <CodeEditor title="3단계: 텍스트 기초분석" initialCode={step3} />

            <h2>4단계: 다중 시각화</h2>
            <p>리뷰 수/평점 콤보, 감성 분석, 리뷰 길이 분포, 주간 추이를 종합 대시보드로 구성합니다.</p>
            <CodeEditor title="4단계: 종합 대시보드" initialCode={step4} />

            <div className="callout-box">
              <h3>실습 과제</h3>
              <ul>
                <li>특정 상품(예: 스마트폰)의 리뷰만 필터링하여 단어 빈도를 분석해보세요.</li>
                <li>평점 1~2점 리뷰에서 자주 등장하는 단어와 4~5점 리뷰의 단어를 비교해보세요.</li>
                <li>리뷰 길이와 평점의 상관관계를 산점도와 상관계수로 분석해보세요.</li>
              </ul>
            </div>

            <div className="lesson-nav">
              <Link to="/projects/weather-pattern" className="lesson-nav-btn prev">&larr; 이전: 날씨 패턴 분석</Link>
              <Link to="/projects/hr-analytics" className="lesson-nav-btn next">다음: 인사 데이터 분석 &rarr;</Link>
            </div>
          </div>
        </div>
      </section>
