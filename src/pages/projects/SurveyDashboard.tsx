...
const step4 = `fig, axes = plt.subplots(1, 3, figsize=(15, 5))
...
print(f"최고 NPS 연령대: {nps_by_age.idxmax()} ({nps_by_age.max():.1f}점)")`

export default function SurveyDashboard() {
  return (
    <>
      <SEOHead title="설문조사 대시보드" description="크로스탭과 범주형 데이터로 설문조사 대시보드를 구성합니다." />
      <section className="page-header">
        <div className="container">
          <h1>설문조사 대시보드</h1>
          <p>크로스탭, 범주형 데이터, 파이 차트로 설문 결과를 분석합니다</p>
        </div>
      </section>
        ...
        <div className="container">
          <div className="playground-body">
            ...
            <div className="callout-box">
              <h3>프로젝트 목표</h3>
              <p>200명의 설문 응답 데이터를 생성하고, 크로스탭 분석과 다양한 파이/바 차트로 대시보드를 구성합니다.
              <strong> 난이도: 중급</strong> | 핵심 스킬: 크로스탭, 범주형 데이터, 파이 차트</p>
            </div>

            <h2>데이터 설명</h2>
            <p>200명의 설문 응답 데이터입니다. 연령대, 성별, 만족도, 유입경로, 재방문 의향, NPS 점수를 포함합니다.</p>

            <h2>1단계: 데이터 생성 및 탐색</h2>
            <p>범주형 데이터를 생성하고 각 항목의 분포를 확인합니다.</p>
            <CodeEditor title="1단계: 데이터 생성 및 탐색" initialCode={step1} />

            <h2>2단계: 크로스탭 분석</h2>
            <p>두 변수 간의 교차 빈도표를 만들어 관계를 파악합니다.</p>
            <CodeEditor title="2단계: 크로스탭 분석" initialCode={step2} />

            <h2>3단계: 범주형 시각화</h2>
            <p>파이 차트, 스택 바 차트, 히스토그램으로 범주형 데이터를 시각화합니다.</p>
            <CodeEditor title="3단계: 범주형 시각화" initialCode={step3} />

            <h2>4단계: 종합 대시보드</h2>
            <p>연령대별 NPS, 유입경로별 재방문율, 만족도별 NPS를 종합 대시보드로 구성합니다.</p>
            <CodeEditor title="4단계: 종합 대시보드" initialCode={step4} />

            <div className="callout-box">
              <h3>실습 과제</h3>
              <ul>
                <li>성별과 만족도의 크로스탭을 만들고 해석해보세요.</li>
                <li>재방문 의향이 '예'인 그룹과 '아니오'인 그룹의 NPS 점수 분포를 비교해보세요.</li>
                <li>연령대별 유입경로 비율을 스택 바 차트로 시각화해보세요.</li>
              </ul>
            </div>

            <div className="lesson-nav">
              <Link to="/projects/student-scores" className="lesson-nav-btn prev">&larr; 이전: 학생 성적 분석</Link>
              <Link to="/projects/weather-pattern" className="lesson-nav-btn next">다음: 날씨 패턴 분석 &rarr;</Link>
            </div>
          </div>
        </div>
      </section>
