...
export default function TimeSeries() {
  return (
    <>
      <SEOHead title="시계열 분석" description="시간에 따른 데이터 패턴을 분석하고 예측하는 방법을 학습합니다." />
      <section className="page-header"><div className="container"><h1>시계열 분석</h1><p>시간에 따른 데이터 패턴을 분석하고 예측하는 방법을 학습합니다</p></div></section>
      <section className="section lesson-content">
        <div className="container">
          <div className="lesson-body">

            <h2>시계열 데이터란?</h2>
            <p>시계열(Time Series) 데이터는 시간 순서에 따라 측정된 데이터입니다. 주가, 매출, 온도, 트래픽 등이 대표적입니다. 추세(Trend), 계절성(Seasonality), 잔차(Residual)를 분해하여 분석합니다.</p>
...
print(f"30일 후 예측 매출: {predictions[-1]:,.0f}")`}</code></pre>
            </div>

            <div className="callout-box">
              <h3>시계열 분석 핵심 포인트</h3>
              <p>시계열 데이터는 반드시 시간 순서를 유지해야 합니다. 이동평균으로 노이즈를 제거하고 추세를 파악하세요. 계절성이 있다면 분해 후 분석하는 것이 효과적입니다.</p>
            </div>
