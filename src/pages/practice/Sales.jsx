...
const step2 = `# 결측치/이상치 확인 및 파생변수 생성
print("[결측치 확인]")
print(df.isnull().sum())
...
print(f"\\n[파생변수 추가 후]")
print(df[['날짜', '월', '요일', '주말', '카테고리', '매출']].head())`
...
const step6 = `import matplotlib.pyplot as plt
...
export default function Sales() {
  return (
    <>
      <SEOHead title="매출 데이터 분석" description="실제 매출 데이터를 활용한 종합 분석 프로젝트를 수행합니다." />
      <section className="page-header">
        <div className="container">
          <h1>매출 데이터 분석</h1>
          <p>실제 매출 데이터를 활용한 종합 분석 프로젝트를 수행합니다</p>
        </div>
      </section>
        ...
        <div className="container">
          <div className="playground-body">
            ...
            <div className="callout-box">
              <h3>프로젝트 목표</h3>
              <p>6개월간의 온라인 쇼핑몰 매출 데이터를 생성하고, 매출 트렌드, 카테고리 분석, 고객 분석, 요일 패턴, 종합 대시보드를 만듭니다.</p>
            </div>

            <h2>데이터 설명</h2>
            <p>가상의 온라인 쇼핑몰 6개월(180일) 매출 데이터입니다:</p>
            <ul>
              <li><strong>날짜</strong> — 주문 날짜 (2024-01-01 ~ 2024-06-28)</li>
              <li><strong>카테고리</strong> — 전자제품, 의류, 식품, 가구, 도서</li>
              <li><strong>채널</strong> — 온라인, 오프라인, 모바일</li>
              <li><strong>지역</strong> — 서울, 경기, 부산, 대구, 인천</li>
              <li><strong>단가/수량/매출</strong> — 매출 = 단가 x 수량</li>
            </ul>

            <h2>1단계: 데이터 생성</h2>
            <p>numpy로 랜덤 데이터를 생성하고 기본 구조를 확인합니다.</p>
            <CodeEditor title="1단계: 데이터 생성" initialCode={step1} />

            <h2>2단계: 전처리</h2>
            <p>결측치/이상치를 확인하고 날짜 파생변수를 생성합니다.</p>
            <CodeEditor title="2단계: 전처리" initialCode={step2} />

            <h2>3단계: 매출 트렌드</h2>
            <p>월별/일별 매출 추이와 이동평균으로 트렌드를 파악합니다.</p>
            <CodeEditor title="3단계: 매출 트렌드" initialCode={step3} />

            <h2>4단계: 카테고리 분석</h2>
            <p>카테고리별 매출 비중과 월별 변화를 분석합니다.</p>
            <CodeEditor title="4단계: 카테고리 분석" initialCode={step4} />

            <h2>5단계: 고객 분석</h2>
            <p>고객별 구매 패턴과 상위 고객을 분석합니다.</p>
            <CodeEditor title="5단계: 고객 분석" initialCode={step5} />

            <h2>6단계: 요일 패턴</h2>
            <p>요일별/채널별 매출 패턴을 파악합니다.</p>
            <CodeEditor title="6단계: 요일 패턴" initialCode={step6} />

            <h2>7단계: 종합 대시보드</h2>
            <p>모든 분석 결과를 하나의 대시보드로 구성합니다.</p>
            <CodeEditor title="7단계: 종합 대시보드" initialCode={step7} />

            <div className="lesson-nav">
              <Link to="/learn/regression" className="lesson-nav-btn prev">&larr; 이전: 회귀 분석</Link>
