...
print(f"골든크로스 (상승 신호) 횟수: {crossover.sum()}")`
...
const step5 = `import numpy as np
import pandas as pd
...
print(f"90일 후 예측 매출: {y_future[-1]:.1f}")`
...
export default function TimeSeries() {
  return (
    <>
      <SEOHead title="시계열 데이터 분석" description="이동평균, 시계열 분해, 추세 예측으로 시계열 데이터를 분석합니다." />
      <section className="page-header">
        <div className="container">
          <h1>시계열 데이터 분석</h1>
          <p>이동평균, 시계열 분해, 추세 예측으로 시계열 데이터를 분석합니다</p>
        </div>
      </section>
        ...
        <div className="container">
          <div className="playground-body">
            ...
            <div className="callout-box">
              <h3>프로젝트 목표</h3>
              <p>2년간의 일별 매출 데이터를 생성하고, 시각화, 이동평균, 시계열 분해, 정상성 검정, 추세 예측, 종합 대시보드를 구성합니다.</p>
            </div>

            <h2>데이터 설명</h2>
            <ul>
              <li><strong>기간</strong> — 2022-01-01 ~ 2023-12-31 (730일)</li>
              <li><strong>매출</strong> — 추세 + 계절성 + 주간패턴 + 노이즈로 구성</li>
            </ul>
            ...
            <h2>2단계: 시각화</h2>
            <CodeEditor title="2단계: 시각화" initialCode={step2} />

            <h2>3단계: 이동평균</h2>
            <CodeEditor title="3단계: 이동평균" initialCode={step3} />

            <h2>4단계: 시계열 분해</h2>
            <CodeEditor title="4단계: 시계열 분해" initialCode={step4} />

            <h2>5단계: 정상성 검정</h2>
            <CodeEditor title="5단계: 정상성 검정" initialCode={step5} />

            <h2>6단계: 추세 예측</h2>
            <CodeEditor title="6단계: 추세 예측" initialCode={step6} />

            <h2>7단계: 종합 대시보드</h2>
            <CodeEditor title="7단계: 종합 대시보드" initialCode={step7} />

            <div className="lesson-nav">
              <Link to="/practice/survey" className="lesson-nav-btn prev">&larr; 이전: 설문 데이터 분석</Link>
              <Link to="/practice/report" className="lesson-nav-btn next">다음: 보고서 자동 생성 &rarr;</Link>
            </div>
          </div>
        </div>
      </section>
