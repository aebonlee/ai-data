...
print(html[:500])
print("...")
print(f"\\n총 {len(html)}자 HTML 생성 완료")`

const step5 = `# 보고서 템플릿 시스템
import pandas as pd
...
# 보고서 생성
print(generate_report(df, "2024 상반기 매출 분석 보고서"))`

const step6 = `# ChatGPT 연동 프롬프트 생성
import pandas as pd
...
for qtype in ["summary", "forecast", "strategy"]:
    ...
    print(f"{'='*50}")
    print(create_chatgpt_prompt(df, qtype))`

const step7 = `# 자동화 파이프라인
import pandas as pd
import numpy as np
    ...
    <>
      ...
      <section className="page-header">
        <div className="container">
          <h1>보고서 자동 생성</h1>
          <p>AI를 활용하여 분석 보고서를 자동으로 생성합니다</p>
        </div>
      </section>
        ...
        <div className="container">
          <div className="playground-body">
            ...
            <div className="callout-box">
              <h3>프로젝트 목표</h3>
              <p>데이터에서 KPI를 계산하고, 차트를 생성하고, HTML 보고서를 만들고, 템플릿 시스템과 ChatGPT 연동, 자동화 파이프라인까지 구축합니다.</p>
            </div>

            <h2>데이터 설명</h2>
            <ul>
              <li><strong>날짜</strong> — 2024-01-01 ~ 2024-06-28 (180일)</li>
              <li><strong>카테고리</strong> — 전자제품, 의류, 식품, 가구, 도서</li>
              <li><strong>매출/주문수</strong> — 일별 카테고리별 매출과 주문 수</li>
            </ul>

            <h2>1단계: 데이터 준비</h2>
            <CodeEditor title="1단계: 데이터 준비" initialCode={step1} />

            <h2>2단계: KPI 계산</h2>
            <CodeEditor title="2단계: KPI 계산" initialCode={step2} />

            <h2>3단계: 차트 생성</h2>
            <CodeEditor title="3단계: 차트 생성" initialCode={step3} />

            <h2>4단계: HTML 보고서</h2>
            <p>분석 결과를 HTML 형식의 보고서로 구성합니다.</p>
            <CodeEditor title="4단계: HTML 보고서" initialCode={step4} />

            <h2>5단계: 템플릿 시스템</h2>
            <p>재사용 가능한 보고서 생성 함수를 만듭니다.</p>
            <CodeEditor title="5단계: 템플릿 시스템" initialCode={step5} />

            <h2>6단계: ChatGPT 연동</h2>
            <p>데이터 요약을 ChatGPT 프롬프트로 변환하여 인사이트를 자동 생성합니다.</p>
            <CodeEditor title="6단계: ChatGPT 연동 프롬프트" initialCode={step6} />

            <h2>7단계: 자동화 파이프라인</h2>
            <p>전체 분석 과정을 하나의 파이프라인으로 자동화합니다.</p>
            <CodeEditor title="7단계: 자동화 파이프라인" initialCode={step7} />

            <div className="lesson-nav">
              <Link to="/practice/timeseries" className="lesson-nav-btn prev">&larr; 이전: 시계열 데이터 분석</Link>
